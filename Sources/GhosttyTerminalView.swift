...
func ctShouldUseTransparentBackgroundWindow() -> Bool {
    ...
    return ctShouldApplyWindowGlass(
...
func ctShouldUseClearWindowBackground(for opacity: Double) -> Bool {
    ctShouldUseTransparentBackgroundWindow() || opacity < 0.999
}

private func ctTransparentWindowBaseColor() -> NSColor {
...
func ctGhosttyModifierActionForFlagsChanged(
    keyCode: UInt16,
...
private func ctRuntimeReadClipboardCallback(
...
#if DEBUG
private func ctChildExitProbePath() -> String? {
    let env = ProcessInfo.processInfo.environment
    guard env["CT_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP"] == "1",
          let path = env["CT_UI_TEST_CHILD_EXIT_KEYBOARD_PATH"],
          !path.isEmpty else {
        return nil
    }
    return path
...
private func ctLoadChildExitProbe(at path: String) -> [String: String] {
...
private func ctWriteChildExitProbe(_ updates: [String: String], increments: [String: Int] = [:]) {
    guard let path = ctChildExitProbePath() else { return }
    var payload = ctLoadChildExitProbe(at: path)
...
private func ctScalarHex(_ value: String?) -> String {
...
private func ctResolveQuicklookPath(
    _ rawText: String,
    cwd: String?,
    ...
    for token in ctQuicklookPathCandidates(from: trimmed) {
...
private func ctQuicklookPathCandidates(from rawText: String) -> [String] {
    ...
    func append(_ candidate: String?) {
        ...
        let punctuationTrimmed = ctTrimTerminalPathTrailingPunctuation(trimmed)
    ...
    let unescaped = ctUnescapeShellToken(rawText)
    if unescaped != rawText {
        append(unescaped)
    }

    if let unquoted = ctUnquoteShellToken(rawText) {
        append(unquoted)
        let unescapedUnquoted = ctUnescapeShellToken(unquoted)
...
private let ctTerminalPathSentencePunctuation: Set<Character> = [
    ".", ",", ";", ":", "!", "?"
]

private let ctTerminalPathTrailingQuotes: Set<Character> = [
    "\"", "'", "”", "’", "»"
]

private let ctTerminalPathClosingPairs: [Character: Character] = [
    ")": "(",
    "]": "[",
    "}": "{",
    ">": "<"
...
private func ctTrimTerminalPathTrailingPunctuation(_ token: String) -> String {
    ...
    while end > 0 {
        ...
        if ctTerminalPathSentencePunctuation.contains(trailing) ||
            ctTerminalPathTrailingQuotes.contains(trailing) {
            end -= 1
            continue
        }

        if let opener = ctTerminalPathClosingPairs[trailing],
           !ctHasUnmatchedOpeningPathDelimiter(
               in: characters[..<(end - 1)],
               opener: opener,
               closer: trailing
           ) {
...
private func ctHasUnmatchedOpeningPathDelimiter(
...
private func ctUnquoteShellToken(_ token: String) -> String? {
    guard token.count >= 2,
...
private func ctUnescapeShellToken(_ token: String) -> String {
...
private func ctVisibleTerminalLines(from text: String, rows: Int) -> [String] {
...
private func ctShellEscapedTokenContainingColumn(
    in line: String,
    column: Int
) -> String? {
...
private func ctIsHardPathDelimiter(
...
private func ctRawPathSegmentContainingColumn(
    in line: String,
    column: Int
) -> String? {
    let characters = Array(line)
    guard !characters.isEmpty, column >= 0, column < characters.count else { return nil }
    guard !ctIsHardPathDelimiter(in: characters, at: column) else { return nil }

    var start = column
    while start > 0, !ctIsHardPathDelimiter(in: characters, at: start - 1) {
        start -= 1
    }

    var end = column
    while (end + 1) < characters.count, !ctIsHardPathDelimiter(in: characters, at: end + 1) {
        end += 1
    }
...
private func ctPathCandidatesContainingColumn(
    in line: String,
    column: Int
) -> [String] {
    var candidates: [String] = []
    ...
    append(ctRawPathSegmentContainingColumn(in: line, column: column))
    append(ctShellEscapedTokenContainingColumn(in: line, column: column))

    return candidates
}

private func ctResolveVisibleLinePath(
    _ line: String,
    column: Int,
    cwd: String,
    fileExists: (String) -> Bool = { FileManager.default.fileExists(atPath: $0) }
) -> (rawToken: String, path: String)? {
    for rawToken in ctPathCandidatesContainingColumn(in: line, column: column) {
        if let resolvedPath = ctResolveQuicklookPath(rawToken, cwd: cwd, fileExists: fileExists) {
...
class GhosttyApp {
    ...
    private static let releaseBundleIdentifier = "com.ctterm.app"
    ...
    private static func resolveBackgroundLogURL(
        ...
        if let explicitPath = environment["CT_DEBUG_BG_LOG"],
           !explicitPath.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return URL(fileURLWithPath: explicitPath)
        }

        if let debugLogPath = environment["CT_DEBUG_LOG"],
        ...
        return URL(fileURLWithPath: "/tmp/ct-bg.log")
    }
                ...
                TerminalImageTransferPlanner.execute(
                    ...
                    uploadWorkspaceRemote: { fileURLs, operation, finish in
                        guard let workspace = MainActor.assumeIsolated({
                            ...
                            finish(.failure(NSError(domain: "ct.remote.paste", code: 3)))
    ...
    let backgroundLogEnabled = {
        if ProcessInfo.processInfo.environment["CT_DEBUG_BG"] == "1" {
            return true
        }
        if ProcessInfo.processInfo.environment["CT_DEBUG_LOG"] != nil {
            return true
        }
        if ProcessInfo.processInfo.environment["GHOSTTYTABS_DEBUG_BG"] == "1" {
            return true
        }
        if UserDefaults.standard.bool(forKey: "ctDebugBG") {
            return true
        }
    ...
    private func endScrollSession() {
        ...
        if scrollLagSampleCount > 0 {
            ...
            if Self.shouldCaptureScrollLagEvent(
                ...
                // Scroll lag detected — telemetry removed
    ...
    #if DEBUG
    private static let initLogPath = "/tmp/ct-ghostty-init.log"
    ...
    private func initializeGhostty() {
        ...
        runtimeConfig.read_clipboard_cb = unsafeBitCast(
            ctRuntimeReadClipboardCallback as @convention(c) (
        ...
        if let created = ghostty_app_new(&runtimeConfig, primaryConfig) {
            ...
            // ct still launches with working terminals.
            ...
            loadInlineGhosttyConfig(
                ...
                into: fallbackConfig,
                prefix: "ct-shell-integration-override",
    ...
    private func loadBundledThemeIfNeeded(_ config: ghostty_config_t) {
        let values = GhosttyConfigFile.readValues(for: ["theme", "background"])
        if let themeName = values["theme"], !themeName.isEmpty,
           let themeURL = Bundle.main.resourceURL?
            .appendingPathComponent("ghostty/themes/\(themeName)"),
           FileManager.default.fileExists(atPath: themeURL.path) {
            themeURL.path.withCString { path in
                ghostty_config_load_file(config, path)
            }
        }
        if let bg = values["background"], !bg.isEmpty {
            loadInlineGhosttyConfig(
                "background = \(bg)",
                into: config,
                prefix: "user-background-override",
                logLabel: "user background override"
            )
        }
    }
    ...
    private func loadDefaultConfigFilesWithLegacyFallback(_ config: ghostty_config_t) {
        if NSApp?.effectiveAppearance.bestMatch(from: [.darkAqua, .aqua]) != .darkAqua {
            loadInlineGhosttyConfig(
                ...
                """,
                into: config,
                prefix: "claude-terminal-light-defaults",
                logLabel: "light mode defaults"
            )
        }
        ghostty_config_load_default_files(config)
        loadLegacyGhosttyConfigIfNeeded(config)
        ghostty_config_load_recursive_files(config)
        loadCtAppSupportGhosttyConfigIfNeeded(config)
        loadBundledThemeIfNeeded(config)
        ...
        if !useHostLayerBackground {
            ...
            loadInlineGhosttyConfig(
                ...
                into: config,
                prefix: "ct-layer-bg-image-override",
                logLabel: "layer background image override"
            )
        } else {
            // ct provides the terminal background via backgroundView (CALayer)
        ...
        // Prevent Ghostty from overriding ZDOTDIR — ct handles shell
    ...
    ///
    /// See: https://github.com/manaflow-ai/ct/pull/1017
    ...
    /// font already covers the ranges ct would otherwise auto-map.
    ...
    /// Returns only the CJK mappings ct should auto-inject after respecting
    ...
    /// application-support config paths that ct may load at runtime.
    ...
    /// Returns the top-level config paths that ct will actually load before
    ...
    static func loadedCJKScanPaths(
        ...
        let appSupportConfigURLs = ctAppSupportConfigURLs(
    ...
    /// Scans a single config file for font settings relevant to ct's
    ...
    static func ctAppSupportConfigURLs(
    ...
    private func loadCtAppSupportGhosttyConfigIfNeeded(_ config: ghostty_config_t) {
        #if os(macOS)
        ...
        let urls = Self.ctAppSupportConfigURLs(
...
#if DEBUG
        dlog(
            "loaded ct app support ghostty config from: \(urls.map(\.path).joined(separator: ", "))"
        )
#endif
        #endif
    }
    ...
    private func handleAction(target: ghostty_target_s, action: ghostty_action_s) -> Bool {
        ...
        if action.tag == GHOSTTY_ACTION_SHOW_CHILD_EXITED {
            ...
            // handles this action. For ct, the correct behavior is to close
        ...
        case GHOSTTY_ACTION_OPEN_URL:
            ...
            // Route markdown file URLs into the ct viewer when the toggle is
            ...
            if !BrowserLinkOpenSettings.openTerminalLinksInCtBrowser() {
                #if DEBUG
                dlog("link.openURL ctBrowser=disabled, opening externally url=\(target.url)")
                #endif
                return performOnMain {
    ...
    private func applyBackgroundToKeyWindow() {
        ...
        if ctShouldUseClearWindowBackground(for: defaultBackgroundOpacity) {
            window.backgroundColor = ctTransparentWindowBaseColor()
    ...
    private func activeMainWindow() -> NSWindow? {
        ...
        if let raw = keyWindow?.identifier?.rawValue,
           raw == "ct.main" || raw.hasPrefix("ct.main.") {
            return keyWindow
        }
        return NSApp.windows.first(where: { window in
            guard let raw = window.identifier?.rawValue else { return false }
            return raw == "ct.main" || raw.hasPrefix("ct.main.")
        })
    }
    ...
    func logBackground(_ message: String) {
        ...
        let line =
            "\(timestamp) seq=\(sequence) t+\(String(format: "%.3f", uptimeMs))ms thread=\(threadLabel) frame60=\(frame60) frame120=\(frame120) ct bg: \(message)\n"
...
final class TerminalSurface: Identifiable, ObservableObject {
    ...
    /// Port ordinal for CT_PORT range assignment
    var portOrdinal: Int = 0
    /// Snapshotted once per app session so all workspaces use consistent values
    private static let sessionPortBase: Int = {
        let val = UserDefaults.standard.integer(forKey: "ctPortBase")
        return val > 0 ? val : 9100
    }()
    private static let sessionPortRangeSize: Int = {
        let val = UserDefaults.standard.integer(forKey: "ctPortRange")
        return val > 0 ? val : 10
    }()
    private let surfaceContext: ghostty_surface_context_e
    private let configTemplate: CtSurfaceConfigTemplate?
    ...
    init(
        ...
        configTemplate: CtSurfaceConfigTemplate?,
    ...
    func debugSurfaceContextLabel() -> String {
        ctSurfaceContextName(surfaceContext)
    }
    ...
    func liveSurfaceForGhosttyAccess(reason: String) -> ghostty_surface_t? {
        ...
        guard registeredOwnerId == id,
              ctSurfacePointerAppearsLive(surface) else {
...
#if DEBUG
    private static let surfaceLogPath = "/tmp/ct-ghostty-surface.log"
    private static let sizeLogPath = "/tmp/ct-ghostty-size.log"
    ...
    private static func sizeLog(_ message: String) {
        let env = ProcessInfo.processInfo.environment
        guard env["CT_UI_TEST_SPLIT_CLOSE_RIGHT_VISUAL"] == "1" else { return }
        let timestamp = ISO8601DateFormatter().string(from: Date())
        let line = "[\(timestamp)] \(message)\n"
    ...
    private func createSurface(for view: GhosttyNSView) {
        ...
        let baseConfig = configTemplate ?? CtSurfaceConfigTemplate()
        ...
        dlog(
            "zoom.create surface=\(id.uuidString.prefix(5)) context=\(ctSurfaceContextName(surfaceContext)) " +
        ...
        setManagedEnvironmentValue("CT_SURFACE_ID", id.uuidString)
        setManagedEnvironmentValue("CT_WORKSPACE_ID", tabId.uuidString)
        // Backward-compatible shell integration keys used by existing scripts/tests.
        setManagedEnvironmentValue("CT_PANEL_ID", id.uuidString)
        setManagedEnvironmentValue("CT_TAB_ID", tabId.uuidString)
        let socketPath = SocketControlSettings.socketPath()
        setManagedEnvironmentValue("CT_SOCKET_PATH", socketPath)
        // Backward-compatible alias expected by older scripts and third-party integrations.
        setManagedEnvironmentValue("CT_SOCKET", socketPath)
        if let bundledCLIURL = Bundle.main.resourceURL?.appendingPathComponent("bin/ct"),
           FileManager.default.isExecutableFile(atPath: bundledCLIURL.path) {
            setManagedEnvironmentValue("CT_BUNDLED_CLI_PATH", bundledCLIURL.path)
        }
        if let bundleId = Bundle.main.bundleIdentifier, !bundleId.isEmpty {
            setManagedEnvironmentValue("CT_BUNDLE_ID", bundleId)
        }
        ...
        do {
            ...
            setManagedEnvironmentValue("CT_PORT", String(startPort))
            setManagedEnvironmentValue("CT_PORT_END", String(startPort + Self.sessionPortRangeSize - 1))
            setManagedEnvironmentValue("CT_PORT_RANGE", String(Self.sessionPortRangeSize))
        }

        let claudeHooksEnabled = ClaudeCodeIntegrationSettings.hooksEnabled()
        if !claudeHooksEnabled {
            setManagedEnvironmentValue("CT_CLAUDE_HOOKS_DISABLED", "1")
        }
        if let customClaudePath = ClaudeCodeIntegrationSettings.customClaudePath() {
            setManagedEnvironmentValue("CT_CUSTOM_CLAUDE_PATH", customClaudePath)
        }
        if !CursorIntegrationSettings.hooksEnabled() {
            setManagedEnvironmentValue("CT_CURSOR_HOOKS_DISABLED", "1")
        }
        if !GeminiIntegrationSettings.hooksEnabled() {
            setManagedEnvironmentValue("CT_GEMINI_HOOKS_DISABLED", "1")
        }
        ...
        if shellIntegrationEnabled,
           let integrationDir = Bundle.main.resourceURL?.appendingPathComponent("shell-integration").path {
            setManagedEnvironmentValue("CT_SHELL_INTEGRATION", "1")
            setManagedEnvironmentValue("CT_SHELL_INTEGRATION_DIR", integrationDir)
            ...
            if shellName == "zsh" {
                if GhosttyApp.shared.userGhosttyShellIntegrationMode != "none" {
                    setManagedEnvironmentValue("CT_LOAD_GHOSTTY_ZSH_INTEGRATION", "1")
                }
                ...
                if let candidateZdotdir, !candidateZdotdir.isEmpty {
                    ...
                    if !isGhosttyInjected {
                        setManagedEnvironmentValue("CT_ZSH_ZDOTDIR", candidateZdotdir)
                    }
                }
                ...
                if GhosttyApp.shared.userGhosttyShellIntegrationMode != "none" {
                    setManagedEnvironmentValue("CT_LOAD_GHOSTTY_BASH_INTEGRATION", "1")
                }
                // macOS ships /bin/bash 3.2, where Ghostty's automatic bash
                // integration is unsupported and HOME-based wrapper startup is
                // not reliable. Bootstrap ct bash integration on the first
                // interactive prompt instead.
                setManagedEnvironmentValue("PROMPT_COMMAND", """
                unset PROMPT_COMMAND; \
                if [[ "${CT_LOAD_GHOSTTY_BASH_INTEGRATION:-0}" == "1" && -n "${GHOSTTY_RESOURCES_DIR:-}" ]]; then \
                _ct_ghostty_bash="$GHOSTTY_RESOURCES_DIR/shell-integration/bash/ghostty.bash"; \
                [[ -r "$_ct_ghostty_bash" ]] && source "$_ct_ghostty_bash"; \
                fi; \
                if [[ "${CT_SHELL_INTEGRATION:-1}" != "0" && -n "${CT_SHELL_INTEGRATION_DIR:-}" ]]; then \
                _ct_bash_integration="$CT_SHELL_INTEGRATION_DIR/ct-bash-integration.bash"; \
                [[ -r "$_ct_bash_integration" ]] && source "$_ct_bash_integration"; \
                fi; \
                unset _ct_ghostty_bash _ct_bash_integration; \
                if declare -F _ct_prompt_command >/dev/null 2>&1; then _ct_prompt_command; fi
                """)
            }
        }
        ...
        if let inheritedFontPoints = configTemplate?.fontSize,
           inheritedFontPoints > 0 {
            let currentFontPoints = ctCurrentSurfaceFontSizePoints(createdSurface)
            let shouldReapply = {
...
#if DEBUG
        let runtimeFontText = ctCurrentSurfaceFontSizePoints(createdSurface).map {
            String(format: "%.2f", $0)
        } ?? "nil"
        dlog(
            "zoom.create.done surface=\(id.uuidString.prefix(5)) context=\(ctSurfaceContextName(surfaceContext)) " +
...
class GhosttyNSView: NSView, NSUserInterfaceValidations {
    private static let focusDebugEnabled: Bool = {
        if ProcessInfo.processInfo.environment["CT_FOCUS_DEBUG"] == "1" {
            return true
        }
        return UserDefaults.standard.bool(forKey: "ctFocusDebug")
    }()
    ...
    private static let sidebarTabReorderPasteboardType = NSPasteboard.PasteboardType("com.ct.sidebar-tab-reorder")
    ...
    private static let keyLatencyProbeEnabled: Bool = {
        if ProcessInfo.processInfo.environment["CT_KEY_LATENCY_PROBE"] == "1" {
            return true
        }
        return UserDefaults.standard.bool(forKey: "ctKeyLatencyProbe")
    }()
    ...
    func applyWindowBackgroundIfActive() {
        ...
        if ctShouldUseClearWindowBackground(for: color.alphaComponent) {
            window.backgroundColor = ctTransparentWindowBaseColor()
            window.isOpaque = false
        ...
        if GhosttyApp.shared.backgroundLogEnabled {
            let signature = "\(ctShouldUseClearWindowBackground(for: color.alphaComponent) ? "transparent" : color.hexString()):\(String(format: "%.3f", color.alphaComponent))"
            if signature != lastLoggedWindowBackgroundSignature {
                ...
                GhosttyApp.shared.logBackground(
                    "window background applied tab=\(tabId?.uuidString ?? "unknown") surface=\(terminalSurface?.id.uuidString ?? "unknown") source=\(source) override=\(overrideHex) default=\(defaultHex) transparent=\(ctShouldUseClearWindowBackground(for: color.alphaComponent)) color=\(color.hexString()) opacity=\(String(format: "%.3f", color.alphaComponent))"
                )
            }
        }
    }
    ...
    private func recordKeyLatency(path: String, event: NSEvent) {
        ...
        CtTypingTiming.logEventDelay(path: path, event: event)
    }
#endif
    ...
    override func performKeyEquivalent(with event: NSEvent) -> Bool {
#if DEBUG
        let typingTimingStart = CtTypingTiming.start()
        defer {
            CtTypingTiming.logDuration(
        ...
        ctWriteChildExitProbe(
            [
                "probePerformCharsHex": ctScalarHex(event.characters),
                "probePerformCharsIgnoringHex": ctScalarHex(event.charactersIgnoringModifiers),
    ...
    override func keyDown(with event: NSEvent) {
        ...
        defer {
            ...
            CtTypingTiming.logDuration(path: "terminal.keyDown", startedAt: typingTimingStart, event: event)
        }
        ...
        ctWriteChildExitProbe(
            [
                "probeKeyDownCharsHex": ctScalarHex(event.characters),
                "probeKeyDownCharsIgnoringHex": ctScalarHex(event.charactersIgnoringModifiers),
        ...
        if flags.contains(.control) && !flags.contains(.command) && !flags.contains(.option) && !hasMarkedText() {
            ...
            if text.isEmpty {
                ...
                #if DEBUG
                let sendTimingStart = CtTypingTiming.start()
                let ghosttySendStart = ProcessInfo.processInfo.systemUptime
                #endif
            ...
            dlog(
                ...
                "handled=\(handled ? 1 : 0) keyCode=\(event.keyCode) chars=\(ctScalarHex(event.characters)) " +
                "ign=\(ctScalarHex(event.charactersIgnoringModifiers)) mods=\(event.modifierFlags.rawValue)"
            )
#endif
...
#if DEBUG
        let interpretTimingStart = CtTypingTiming.start()
    ...
    private func sendTimedGhosttyKey(
        ...
        let timingStart = CtTypingTiming.start()
        ...
        CtTypingTiming.logDuration(path: path, startedAt: timingStart, event: event, extra: mergedExtra)
        return handled
    }
#endif
    ...
    override func flagsChanged(with event: NSEvent) {
        ...
        if !hasMarkedText(),
           let action = ctGhosttyModifierActionForFlagsChanged(
            keyCode: event.keyCode,
    ...
    private func hoverModsFromFlags(
        ...
        if suppressCommandPathHover, flags.contains(.command) {
            _ = CtUITestCapture.mutateJSONObjectIfConfigured(
                envKey: "CT_UI_TEST_CMD_HOVER_DIAGNOSTICS_PATH"
            ) { payload in
    ...
    private func runtimeDebugLog(
        ...
        CtRuntimeDebugCapture.logIfConfigured(
...
#if DEBUG
                    let resolvedQuicklookWord = ctTerminalCmdClickQuicklookOverride(decodedWord)
#else
                    let resolvedQuicklookWord = decodedWord
#endif
                    if let resolvedPath = ctResolveQuicklookPath(resolvedQuicklookWord, cwd: cwd) {
...
#if DEBUG
                let viewportOffsetStart = ctTerminalCmdClickViewportOffsetDelta(Int(text.offset_start))
#else
    ...
    #if DEBUG
    private func ctTerminalCmdClickQuicklookOverride(_ decodedWord: String) -> String {
        let env = ProcessInfo.processInfo.environment
        guard let override = env["CT_UI_TEST_TERMINAL_CMD_CLICK_QUICKLOOK_OVERRIDE"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
    ...
    private func ctTerminalCmdClickViewportOffsetDelta(_ viewportOffsetStart: Int) -> Int {
        let env = ProcessInfo.processInfo.environment
        guard let delta = env["CT_UI_TEST_TERMINAL_CMD_CLICK_VIEWPORT_OFFSET_DELTA"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
    ...
    private func resolveVisibleWordPathFromViewportOffset(
        ...
        let visibleLines = ctVisibleTerminalLines(from: visibleText, rows: rows)
        ...
        guard let resolution = ctResolveVisibleLinePath(
        ...
        TerminalImageTransferPlanner.execute(
            ...
            uploadWorkspaceRemote: { [weak self] fileURLs, operation, finish in
                guard let workspace = MainActor.assumeIsolated({
                    ...
                    finish(.failure(NSError(domain: "ct.remote.drop", code: 3)))
                    GhosttyPasteboardHelper.cleanupTransferredTemporaryImageFiles(fileURLs)
                    return
                }
                workspace.uploadDroppedFilesForRemoteTerminal(
    ...
    fileprivate func debugSimulateFileDrop(paths: [String]) -> Bool {
        ...
        let pbName = NSPasteboard.Name("ct.debug.drop.\(UUID().uuidString)")
...
final class GhosttySurfaceScrollView: NSView {
    ...
    init(surfaceView: GhosttyNSView) {
        ...
        dropZoneOverlayView.layer?.backgroundColor = ctAccentNSColor().withAlphaComponent(0.25).cgColor
        dropZoneOverlayView.layer?.borderColor = ctAccentNSColor().cgColor
    ...
    func triggerFlash(style: FlashStyle = .navigation) {
        DispatchQueue.main.async { [weak self] in
            ...
            self.flashLayer.add(animation, forKey: "ct.flash")
        }
    }
...
extension GhosttyNSView: NSTextInputClient {
    ...
    fileprivate func sendTextToSurface(_ chars: String, preserveLiteralEscape: Bool) {
        ...
        ctWriteChildExitProbe(
            [
                "probeInsertTextCharsHex": ctScalarHex(chars),
